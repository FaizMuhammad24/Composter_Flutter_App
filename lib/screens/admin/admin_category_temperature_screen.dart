    ...
    _rtdbSubLive = FirebaseDatabase.instance.ref('komposter').onValue.listen((event) {
      if (event.snapshot.value != null && mounted) {
        ...
        setState(() {
          ...
          _currentValue = stale ? 0.0 : ((data['temperature'] as num?)?.toDouble() ?? 0.0);
          final actuators = data['actuators'] as Map? ?? {};
          _heaterStatus = !stale && actuators['heater'] == true;
        });
      }
    });
                            ...
                            child: Column(
                              children: [
                                ...
                                Align(
                                  ...
                                  child: Text(_isOffline ? '--°C' : '${_currentValue.toStringAsFixed(1)}°C', style: const TextStyle(color: Colors.white, fontSize: 48, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                                ),
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(_isOffline ? 'TERPUTUS' : (_heaterStatus ? 'AKTIF (ON)' : 'MATI (OFF)'), style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                                        ],
                                      ),
                                    ),
                                    Container(
                                      ...
                                      child: Text(
                                        _isOffline ? 'Offline' : (_currentValue <= 60 ? 'Normal' : 'Tinggi'),
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildStatRow('Rata-rata', _isOffline ? '-' : '${_calculateAvg().toStringAsFixed(1)}°C'),
                                const Divider(),
                                _buildStatRow('Tertinggi', _isOffline ? '-' : '${_calculateMax().toStringAsFixed(1)}°C'),
                                const Divider(),
                                _buildStatRow('Heater Sedang Aktif', _isOffline ? 'Terputus' : (_heaterStatus ? 'Ya' : 'Tidak')),
                              ],
                            ),
                          ),
                        ),
