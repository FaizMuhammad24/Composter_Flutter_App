  ...
  I-Compost adalah aplikasi mobile Flutter yang menghubungkan **pengguna akhir** dan **operator admin** dalam satu ekosistem pengelolaan kompos pintar. Sistem ini terintegrasi dengan perangkat keras ESP32 yang memantau kondisi komposter secara real-time.
  ...
  `AdminService` membuat instance Firebase kedua (`SecondaryApp`) untuk keperluan CRUD akun admin, agar admin yang sedang login tidak ikut ter-logout saat membuat akun admin baru:
  ...
  | Aspek | User | Admin |
  |-------|------|-------|
  | **Warna Tema** | Hijau (`#2D5016`) | Kuning/Amber (`#FBC02D`) |
  | **Entry Screen** | `UserMainScreen` | `AdminMainScreen` |
  | **Cek di** | `splash_screen.dart` via `SessionService.getRole()` | sama |
  | **Verifikasi Email** | **Wajib** (diblokir jika belum) | Tidak dicek |
  | **Setor Sampah** | ✅ | ❌ |
  | **Monitoring Sensor** | ❌ | ✅ |
  | **Approve Setoran** | ❌ | ✅ |
  | **Kelola Reward** | ❌ | ✅ |
  ...
  | `reset_password_screen.dart` | Kirim link reset password |
  ...
  **Screen tambahan User (push):**
  ...
  | File | Field Utama |
  |------|-------------|
  | `user_model.dart` | `uid`, `name`, `email`, `role` (user/admin), `points`, `photoUrl` |
  ...
  - [ ] Login dengan setiap role (user, admin)
